###############################################
# IMPORT MODULES
###############################################

import numpy as np
from numpy.linalg import norm as norm
from numba import njit

from functions_pf import get_angle
from functions_pf import parallel_transport
from functions_pf import computeEdges
from functions_pf import computeTangents
from functions_pf import computeBishopFrame
from functions_pf import computeMaterialFrame
from functions_pf import computeVoronoiLen
from functions_pf import computeCurvatureBinormals
from functions_pf import computeTwist
from functions_pf import computeK1
from functions_pf import computeK2
from functions_pf import computedK1de
from functions_pf import computedK2de

#from functions_forces import Fstretch
from functions_forces import Fstretch_diss
from functions_forces import Fbend
from functions_forces import Ftwist
from functions_forces import Ftwist_theta
from functions_forces import FcoupleM_k2
from functions_forces import Fcouple_theta2

from functions_forces import Flat



###############################################
# USER-DEFINED SUPPLEMENTARY FUNCTIONS
###############################################

@njit(fastmath=True)
def fill_params(v1, v2, Nt):
    params = np.zeros(Nt+1)

    for i in range(1, Nt):
        if i % 2 != 0:
            params[i] = v1
        else:
            params[i] = v2

    return params



###############################################
# BROWNIAN DYNAMICS MODULE
###############################################
@njit(fastmath=True)
def run_bd_mt(nt, nt_skip, Nt, npf, flag_restart, v_restart, theta_restart, mref_restart, ut_restart, vt_restart, params_diff, params_means, params_stiff):

    kbt = 2.5 # kJ/mol
    R_MT = 12.0 # nm

    ################################
    # Unpacking model parameters
    ################################
    dv2 = params_diff[0]
    dth2 = params_diff[1]

    ht = fill_params(params_means[0], params_means[1], Nt) # nm
    ht[0] = 8.15/2.0 # nm
    K1eq = fill_params(params_means[2], params_means[3], Nt)
    K2eq = fill_params(params_means[4], params_means[5], Nt)
    Mtwist_eq = fill_params(params_means[6], params_means[7], Nt)

    Es = fill_params(params_stiff[0], params_stiff[1], Nt) # kJ/mol/nm
    Ek1 = fill_params(params_stiff[2], params_stiff[3], Nt) # kJ/mol*nm
    Ek2 = fill_params(params_stiff[4], params_stiff[5], Nt) # kJ/mol*nm
    Et = fill_params(params_stiff[6], params_stiff[7], Nt) # kJ/mol*nm
    Etb2 = fill_params(params_stiff[8], params_stiff[9], Nt) # kJ/mol*nm
    epsilon_long = params_stiff[10] # kJ/mol
    a_long = params_stiff[11] # 1/nm
    epsilon_lat_homo = params_stiff[12] # kJ/mol
    epsilon_lat_seam = params_stiff[13] # kJ/mol
    a_lat_homo = params_stiff[14] # 1/nm
    a_lat_seam = params_stiff[15] # 1/nm
    alpha = params_stiff[16]

    ################################
    # Starting configuration
    ################################

    # define necessary arrays
    ed = np.zeros((npf, Nt, 3))
    tang = np.zeros((npf, Nt, 3))
    ut, vt = np.zeros((npf, Nt, 3)), np.zeros((npf, Nt, 3))
    Mtwist = np.zeros((npf, Nt+1))
    lv = np.zeros((npf, Nt+1))
    M1, M2 = np.zeros((npf, Nt, 3)), np.zeros((npf, Nt, 3))
    kb = np.zeros((npf, Nt+1, 3))
    theta = np.zeros((npf, Nt))
    u0 = np.zeros((npf, 3))
    mref = np.zeros((npf, Nt))

    if flag_restart:
        # start from last frame
        v = v_restart
        theta = theta_restart
        ut = ut_restart
        vt = vt_restart
        mref = mref_restart
    else:
        # locate PF positions on a circle
        init_pf_pos = np.zeros((npf, 2))
        for p in range(npf):
            init_pf_pos[p, 0] = R_MT * np.cos(2.0*np.pi*p/14.0)
            init_pf_pos[p, 1] = R_MT * np.sin(2.0*np.pi*p/14.0)

        # create nodes
        v = np.zeros((npf, Nt+1, 3))
        for p in range(npf):
            temp_pos = 0.0
            for i in range(Nt+1):
                v[p, i] = np.array([init_pf_pos[p, 0], init_pf_pos[p, 1], temp_pos - 0.8845*p])
                temp_pos = ht[i] + temp_pos

        # initialize MT model
        for p in range(npf):
            t0 = np.array([0.0, 0.0, 1.0])
            u_tem = -np.array([init_pf_pos[p, 0], init_pf_pos[p, 1], 0.0])
            u0[p] = u_tem/norm(u_tem)

            ed[p] = computeEdges(Nt, v[p])
            tang[p] = computeTangents(Nt, ed[p])
            ut[p], vt[p] = computeBishopFrame(Nt, t0, u0[p], tang[p])
            Mtwist[p] = computeTwist(Nt, theta[p], mref[p])
            M1[p], M2[p] = computeMaterialFrame(Nt, ut[p], vt[p], theta[p])
            lv[p] = computeVoronoiLen(Nt, ed[p])
            kb[p] = computeCurvatureBinormals(Nt, tang[p])
            M1[p, 0] = u0[p]
            M2[p, 0] = np.cross(t0, u0[p])

            ut[p, 0] = u0[p]
            vt[p, 0] = np.cross(t0, u0[p])

    ################################
    # Output arrays
    ################################
    nn = int(nt//nt_skip)

    traj_v = np.zeros((nn, npf, Nt+1, 3))
    traj_dir = np.zeros((nn, npf, Nt, 3))
    traj_theta = np.zeros((nn, npf, Nt))
    traj_U = np.zeros((nn, npf, Nt, 3))
    traj_V = np.zeros((nn, npf, Nt, 3))
    traj_mref = np.zeros((nn, npf, Nt))
    ut_1 = np.zeros((npf, Nt, 3))
    vt_1 = np.zeros((npf, Nt, 3))
    tang_1 = np.zeros((npf, Nt, 3))

    ################################
    # Main time cycle
    ################################
    frame = 0
    for ts in range(nt):
        ################################
        # Update PF
        ################################
        for p in range(npf):
            ed[p] = computeEdges(Nt, v[p])
            tang[p] = computeTangents(Nt, ed[p])

            if ts != 0:
                # parallel transport in time
                for i in range(Nt):
                    ut[p, i] = parallel_transport(ut_1[p, i], tang_1[p, i], tang[p, i])
                    vt[p, i] = parallel_transport(vt_1[p, i], tang_1[p, i], tang[p, i])
                
                # update reference twist
                for i in range(1, Nt):
                    uu = parallel_transport(ut[p, i-1], tang[p, i-1], tang[p, i])
                    mref[p, i] = get_angle(uu, ut[p, i], tang[p, i])

            M1[p], M2[p] = computeMaterialFrame(Nt, ut[p], vt[p], theta[p])
            lv[p] = computeVoronoiLen(Nt, ed[p])
            kb[p] = computeCurvatureBinormals(Nt, tang[p])
            Mtwist[p] = computeTwist(Nt, theta[p], mref[p])

        ################################
        # Compute Forces
        ################################
        # don't calculate lateral forces for a single PF
        if npf > 1:
            flv = Flat(Nt, M1, M2, v, tang, npf, epsilon_lat_homo, epsilon_lat_seam, a_lat_homo, a_lat_seam, alpha)
        else:
            flv = np.zeros((npf, Nt+1, 3))

        for p in range(npf):
            #fs = Fstretch(Nt, ed[p], tang[p], ht, Es)
            fs = Fstretch_diss(Nt, ed[p], tang[p], ht, Es, epsilon_long, a_long)
            fb = Fbend(Nt, M1[p], M2[p], kb[p], tang[p], ed[p], lv[p], K1eq, K2eq, Ek1, Ek2)
            ft = Ftwist(Nt, ed[p], Mtwist[p], kb[p], lv[p], Mtwist_eq, Et)
            ft_theta = Ftwist_theta(Nt, Mtwist[p], lv[p], Mtwist_eq, Et)
            fc2 = FcoupleM_k2(Nt, Mtwist[p], kb[p], lv[p], tang[p], ed[p], M1[p], Mtwist_eq, K2eq, Etb2)
            fc2_theta = Fcouple_theta2(Nt, M1[p], lv[p], kb[p], K2eq, Etb2)

            ################################
            # Update coordinates and angles
            ################################
            # update coordinates
            # first and second nodes are static
            for i in range(2, Nt+1):
                v[p, i] = v[p, i] +\
                          dv2/kbt*(fs[i] + fb[i] + ft[i] + fc2[i] + flv[p, i]) +\
                          np.sqrt(2.0*dv2)*np.array([np.random.normal(),
                                                     np.random.normal(),
                                                     np.random.normal()])

            # update angles
            # first edge rotation angle is static
            for i in range(1, Nt):
                theta[p, i] = theta[p, i] +\
                              dth2/kbt*(ft_theta[i] + fc2_theta[i]) +\
                              np.sqrt(2.0*dth2)*np.random.normal()

            ################################
            # Save for parallel transport
            ################################
            tang_1[p] = tang[p]
            ut_1[p] = ut[p]
            vt_1[p] = vt[p]

        ################################
        # Write trajectory
        ################################
        if ts % nt_skip == 0:
            for p in range(npf):
                for i in range(Nt):
                    traj_dir[frame, p, i] = v[p, i] + (v[p, i+1] - v[p, i])/2.0 + M1[p, i]*2.0

            traj_v[frame] = v
            traj_theta[frame] = theta
            traj_U[frame] = ut
            traj_V[frame] = vt
            traj_mref[frame] = mref

            frame += 1

    return traj_v, traj_dir, traj_theta, traj_U, traj_V, traj_mref

